...
下载`CentOS-7.6-x86_64_190926a.box`文件，保存到 `D:\box_file` 目录。

### ②　将box文件加入本地vagrant环境
在cmd窗口执行命令：
```bash
vagrant box add centos7-64 D:\box_file\CentOS-7.6-x86_64_190926a.box。
```
### ③　初始化虚拟机
新建空目录：D:\centos7，在cmd窗口 cd 到该目录，执行命令：
```bash
